...
void Builder::ugv1_subCallback(const nav_msgs::Odometry::ConstPtr& msgs, const int list_idx){
  ugvout_list[list_idx].pose=msgs->pose.pose;
  
}
void Builder::ugvout_subCallback(const geometry_msgs::PoseStamped::ConstPtr& msgs, const int list_idx){
  ROS_INFO("ugvout_list size: %d,\n list_idx: %d", ugvout_list.size(), list_idx);
  ugvout_list[list_idx].pose=msgs->pose;
  // ROS_INFO(" FOOBAR index: %d, pose.position.x: %lf", list_idx, ugvout_list[list_idx].pose.position.x);
  ...
  :nh_private_("~")
{
	...
	if(gazebo == 1) {
     	for (int i=1; i < n+1; i++){
			std::string sub_topic3 = "/R" + std::to_string(i) + "/odom";
